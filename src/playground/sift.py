...
# img_matches = cv.drawMatches(partColor, kp, imageColor, kp2, matches[:1], imageColor, flags=cv.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)
bestMatch = matches[0]
bmKp1 = kp[bestMatch.queryIdx]
bmKp2 = kp2[bestMatch.trainIdx]
print(bestMatch, f"index first argument: {bestMatch.queryIdx}, index second argument: {bestMatch.trainIdx}")
