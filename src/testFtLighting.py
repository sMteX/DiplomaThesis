...
# load annotations - they contain where each part is
annotations = None
...
# construct absolute paths for all parts
...
progress = 0
step = 5    # step in brightness/contrast in %
total = float(((160 / step) + 1)**2)    # total amount of tests
countDict = { b: {c: 0 for c in range(-80, 81, step)} for b in range(-80, 81, step)}    # constructs dictionary for counts for each combination of brightness and contrast
...
for b in range(-80, 81, step):
    for c in range(-80, 81, step):
        progress += 1
        print(f"({strftime('%H:%M:%S')}) ({formatProgress(progress)}) Brightness: {formatPercentage(b)}, Contrast: {formatPercentage(c)}")
